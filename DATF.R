...
this.queue_size = this.theBackoffEntity.getCurrentQueueSize();
this.discarded = this.theBackoffEntity.getDiscardedCounter();
this.collision = this.theBackoffEntity.getCollisionCount();
this.cur_ent_time = this.theBackoffEntity.getTime().getTimeMs();
// since the current time was observed to increment every 1000 
if (this.rate_of_change_of_queue > 0) {
  if (this.collision >= 0 && this.collision < 100) {
    this.mac.getPhy().setCurrentPhyMode("BPSK12");
    this.mac.getPhy().setCurrentTransmitPower_dBm(-80);
  } else if (this.collision >= 100 && this.collision < 250) {
    this.mac.getPhy().setCurrentPhyMode("BPSK34");
    this.mac.getPhy().setCurrentTransmitPower_dBm(-50);
  } else if (this.collision >= 250 && this.collision < 300) {
    this.mac.getPhy().setCurrentPhyMode("QPSK12");
    this.mac.getPhy().setCurrentTransmitPower_dBm(-30);
  } else if (this.collision >= 300 && this.collision < 450) {
    this.mac.getPhy().setCurrentPhyMode("QPSK34");
    this.mac.getPhy().setCurrentTransmitPower_dBm(-10);
  } else if (this.collision >= 450 && this.collision < 550) {
    this.mac.getPhy().setCurrentPhyMode("16QAM12");
    this.mac.getPhy().setCurrentTransmitPower_dBm(0);

  } else if (this.collision >= 550 && this.collision < 600) {
    this.mac.getPhy().setCurrentPhyMode("16QAM34");
    this.mac.getPhy().setCurrentTransmitPower_dBm(10);

  } else if (this.collision >= 600 && this.collision < 750) {
    this.mac.getPhy().setCurrentPhyMode("64QAM23");
    this.mac.getPhy().setCurrentTransmitPower_dBm(20);
